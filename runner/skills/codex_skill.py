...
ALLOWED_SKILL_AGENTS = {"CoderAgent", "BuilderAgent", "UXAgent"}
...
def run_codex_skill(
    *,
    ...
    if not command_raw.strip():
        command_raw = f"codex exec -C {repo_path} -"
    ...
    manual_required = timed_out or exit_code != 0
    if manual_required:
        reason = (
